...
class SignalGenModel(BehaviorModelExecutor):
    ...
    def output(self):
        ...
        if self._cur_state == "Generate":
            ...
            #color = "red"
            #num = "one"
            num, color = self.event_list[self.order]
            self.order += 1
            ...
            if num not in self.detect_map:
                #print("Dddddd")
                _ho = HealthObject()
                hm = HumanModel(0, Infinite, f"HumanModel[{num}]",
                                "seni_human", num, _ho)
                hc = HumanCheck(0, Infinite, f"HumanCheck[{num}]",
                                "seni_human", num, _ho)
                hw = HumanWeather(0, Infinite, f"HumanWeather[{num}]",
                                  "seni_human", num, _ho)

                self.sys_engine.get_engine("seni_human").insert_input_port(
                    f"info[{num}]")

                self.sys_engine.get_engine("seni_human").register_entity(hm)
                self.sys_engine.get_engine("seni_human").coupling_relation(
                    None, f"info[{num}]", hm, "info")
                self.sys_engine.get_engine("seni_human").coupling_relation(
                    None, f"winfo", hw, "winfo")
            ...
            self.sys_engine.get_engine("seni_human").insert_external_event(
                f"info[{num}]", info)
